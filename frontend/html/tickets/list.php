...
$status = $this->getDataForm('status') ? $this->getDataForm('status') : [];
?>
		...
		<div class="col-sm-8  col-xs-12">
			<ul class="search-status-tabs" role="tablist">
				<li class="status-tab <?php echo $this->isActive('active') ? 'active' : ''; ?>">
					<div class="dropdown">
						<a class="status-tab-link dropdown-toggle" href="<?php echo $this->getPath(['status' => implode(',', [Ticket::read, Ticket::answered, Ticket::unread, Ticket::in_progress])]); ?>" type="button" id="dropdown-tab-active" data-toggle="dropdown" aria-haspopup="true" aria-expanded="true">
						<?php echo t('ticketing.ticket.status.active'); ?>
						<span class="caret"></span>
						</a>
						<ul class="dropdown-menu" aria-labelledby="dropdown-tab-active">
							<li><a href="<?php echo $this->getPath(['status' => implode(',', [Ticket::read, Ticket::answered, Ticket::unread, Ticket::in_progress])]); ?>"><i class="fa fa-check"></i> <?php echo t('ticketing.ticket.status.active.all'); ?></a></li>
							<li><a href="<?php echo $this->getPath(['status' => Ticket::read]); ?>"><i class="fa fa-eye"></i> <?php echo t('read'); ?></a></li>
							<li><a href="<?php echo $this->getPath(['status' => Ticket::answered]); ?>"><i class="fa fa-reply"></i> <?php echo t('answered'); ?></a></li>
							<li><a href="<?php echo $this->getPath(['status' => Ticket::unread]); ?>"><i class="fa fa-eye-slash"></i> <?php echo t('unread'); ?></a></li>
							<li><a href="<?php echo $this->getPath(['status' => Ticket::in_progress]); ?>"><i class="fa fa-spinner fa-spin"></i> <?php echo t('in_progress'); ?></a></li>
						</ul>
					</div>
				</li>
				<li class="status-tab <?php echo $this->isActive('inProgress') ? 'active' : ''; ?>">
					<a class="status-tab-link" href="<?php echo $this->getPath(['status' => Ticket::in_progress]); ?>"><?php echo t('ticket.inprogress'); ?></a>
				</li>
				<li class="status-tab <?php echo $this->isActive('closed') ? 'active' : ''; ?>">
					<a class="status-tab-link" href="<?php echo $this->getPath(['status' => Ticket::closed]); ?>"><?php echo t('closed'); ?></a>
				</li>
				<li class="status-tab <?php echo $this->isActive() ? 'active' : ''; ?>">
					<a class="status-tab-link" href="<?php echo $this->getPath(['status' => implode(',', [Ticket::unread, Ticket::read, Ticket::in_progress, Ticket::answered, Ticket::closed])]); ?>"><?php echo t('ticketing.ticket.all'); ?></a>
				</li>
			</ul>
		</div>
		<?php if ($this->canAdd) { ?>
    ...
    ?>
				...
				<?php if ($this->multiuser) { ?>
					<span>
						...
						<a class="client" href="<?php echo userpanel\url('users', ['id' => $ticket->client->id]); ?>"><?php echo $ticket->client->getFullName(); ?></a>
					</span>
				<?php } ?>
			...
			<div class="col-sm-4 col-xs-12 ticket-info">
				...
				<?php if ($ticket->operator_id) { ?>
					<span>
						<a target="_blank" href="<?php echo $this->hasAccessToUsers ? userpanel\url('users', ['id' => $ticket->operator_id]) : 'javascript::void()'; ?>" class="tooltips" title="<?php echo $ticket->operator->getFullName(); ?>">
							<img class="img-circle" src="<?php echo $ticket->operator->getAvatar(16, 16); ?>" alt="<?php echo $ticket->operator->getFullName(); ?>">
						</a>
					</span>
				<?php
				}
    $priorityClass = Utility::switchcase($ticket->priority, [
        'label label-warning' => Ticket::instantaneous,
        'label label-primary' => Ticket::important,
        'label label-info' => Ticket::ordinary,
    ]);
    $priorityTxt = Utility::switchcase($ticket->priority, [
